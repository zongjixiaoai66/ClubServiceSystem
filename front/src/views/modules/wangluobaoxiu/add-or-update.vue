    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="报修地址" prop="baoxiudizhi">
          <el-input v-model="ruleForm.baoxiudizhi" 
              placeholder="报修地址" clearable  :readonly="ro.baoxiudizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="报修地址" prop="baoxiudizhi">
              <el-input v-model="ruleForm.baoxiudizhi" 
                placeholder="报修地址" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="故障说明" prop="guzhangshuoming">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.guzhangshuoming" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.guzhangshuoming" label="故障说明" prop="guzhangshuoming">
                    <span v-html="ruleForm.guzhangshuoming"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.baoxiushijian = true;
          ...
          if(o=='guzhangshuoming'){
            this.ruleForm.guzhangshuoming = obj[o];
	    this.ro.guzhangshuoming = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.guzhangshuoming = this.ruleForm.guzhangshuoming.replace(reg,'../../../springboot48zml/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wangluobaoxiu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wangluobaoxiuCrossAddOrUpdateFlag = false;
						 this.parent.search();
