...
export default {
  ...
  methods: {
	...
	this.ruleForm.taocanxiangqing = this.ruleForm.taocanxiangqing.replace(reg,'../../../springboot48zml/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wangluotaocan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wangluotaocanCrossAddOrUpdateFlag = false;
						 this.parent.search();
